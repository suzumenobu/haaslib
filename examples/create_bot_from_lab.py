...
def main():
    ...
    backtest4bot = backtesting_result.items[0]
    bot_creation_resp = api.add_bot_from_lab(
        executor,
        AddBotFromLabRequest(
            lab_id=backtest4bot.lab_id,
            backtest_id=backtest4bot.backtest_id,
            bot_name="Test bot from lab backtest",
            account_id=account.account_id,
            market=market,
        ),
    )

    print(f"{bot_creation_resp=}")
